...
(() => {
  ...
  // src/components/WebGLGrid/WebGLGrid.ts
  var WebGLGrid = class _WebGLGrid {
    constructor(canvas) {
      this.images = [];
  ...
  `;
      this.render = () => {
        ...
        const { gl } = this;
        if (!this.isDragging && (Math.abs(this.momentum.x) > 0.01 || Math.abs(this.momentum.y) > 0.01)) {
    ...
    static async create(canvas, options) {
      const instance = new _WebGLGrid(canvas);
    ...
    transitions: [
      {
        ...
        enter(data) {
          ...
          const tl = gsapWithCSS.to(data.next.container, {
            x: 0,
            duration: 1.5,
            ease: "expo.inOut",
            onComplete: () => {
